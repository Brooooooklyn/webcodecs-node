...
import { VideoEncoder, VideoDecoder } from '../../index.js'
import {
...
for (const codec of getAllCodecs()) {
  ...
  test(`codec support: ${codecInfo.name} encoder isConfigSupported`, (t) => {
  ...
  test(`codec support: ${codecInfo.name} decoder isConfigSupported`, (t) => {
...
test('codec: H.264 encode-decode roundtrip', (t) => {
  const width = 320
  const height = 240
...
test('codec: VP8 encode-decode roundtrip', (t) => {
  const width = 320
  const height = 240

  // Check if VP8 is supported
  const support = VideoEncoder.isConfigSupported(createEncoderConfig('vp8', width, height))
...
test('codec: VP9 encode-decode roundtrip', (t) => {
  const width = 320
  const height = 240

  // Check if VP9 is supported
  const support = VideoEncoder.isConfigSupported(createEncoderConfig('vp9', width, height))
  if (!support.supported) {
...
test('codec: H.265 encode-decode roundtrip', (t) => {
  const width = 320
  const height = 240

  // Check if H.265 is supported
  const support = VideoEncoder.isConfigSupported(createEncoderConfig('h265', width, height))
  if (!support.supported) {
...
test('codec: AV1 encode-decode roundtrip', (t) => {
  const width = 320
  const height = 240

  // Check if AV1 is supported
  const support = VideoEncoder.isConfigSupported(createEncoderConfig('av1', width, height))
  if (!support.supported) {
...
for (const res of resolutions) {
  test(`codec matrix: H.264 @ ${res.name} (${res.width}x${res.height})`, (t) => {
...
test('codec comparison: quality across codecs', (t) => {
  const width = 320
  const height = 240
  const original = generateGradientI420Frame(width, height, 0)
  const originalData = extractI420Data(original)
  ...
  for (const codec of codecs) {
    const support = VideoEncoder.isConfigSupported(createEncoderConfig(codec, width, height))
    if (!support.supported) {
