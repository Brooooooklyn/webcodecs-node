...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./webcodecs.wasi.cjs', false, [
        './webcodecs.wasm32-wasi.debug.wasm',
        './webcodecs.wasm32-wasi.wasm',
      ])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@napi-rs/webcodecs-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          ...
          if (bindingPackageVersion !== '1.3.1') {
            throw new Error(
              `WASI binding package version mismatch, expected 1.3.1 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`,
            )
          }
        }
