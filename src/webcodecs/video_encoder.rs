...
struct VideoEncoderInner {
  ...
  /// Whether the encoder entered the closed state because encoding failed.
  /// Used to distinguish EncodingError from an explicit close in flush().
  had_error: bool,
...
impl VideoEncoder {
  ...
  fn process_encode(
    ...
    if guard.input_color_space.is_none() {
      guard.input_color_space = color_space;
    }
  ...
  fn report_error(inner: &mut VideoEncoderInner, error_msg: &str) {
    ...
    tracing::warn!(target: "webcodecs", codec = "VideoEncoder", error = error_msg, "Codec error reported");
  ...
  #[napi(
    ts_args_type = "eventType: string, callback: (event: Event) => unknown, options?: AddEventListenerOptions | undefined | null"
  )]
  ...
  #[napi(
    ts_args_type = "eventType: string, callback: (event: Event) => unknown, options?: EventListenerOptions | undefined | null"
  )]
