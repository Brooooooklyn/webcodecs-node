...
impl std::fmt::Debug for Scaler {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Scaler")
            .field("src", &format!("{}x{} {:?}", self.src_width, self.src_height, self.src_format))
            .field("dst", &format!("{}x{} {:?}", self.dst_width, self.dst_height, self.dst_format))
            .finish()
    }
}
...
mod tests {
    ...
    fn test_scaler_creation() {
        let scaler = Scaler::new(
            ...
            1280, 720, AVPixelFormat::Yuv420p,
            ScaleAlgorithm::Bilinear,
        );
        assert!(scaler.is_ok());
    }
    ...
    fn test_converter_creation() {
        let converter = Scaler::new_converter(
            1920, 1080,
            AVPixelFormat::Rgba,
