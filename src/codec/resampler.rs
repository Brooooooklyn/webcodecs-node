...
impl Resampler {
  ...
  pub fn new(
    ...
    let ptr = NonNull::new(ctx)
      .ok_or_else(|| CodecError::InvalidConfig("Internal error: ctx should not be null".into()))?;
  ...
  pub fn convert(&mut self, src: &Frame, dst: &mut Frame) -> CodecResult<u32> {
    if !src.is_audio() {
      return Err(CodecError::InvalidConfig(
        "Source is not an audio frame".into(),
      ));
    }
...
mod tests {
  ...
  fn test_rate_converter_creation() {
    let converter = Resampler::new_rate_converter(2, 44100, 48000, AVSampleFormat::S16);
    assert!(
      converter.is_ok(),
      "Rate converter creation failed: {:?}",
      converter.err()
    );
  ...
  fn test_format_converter_creation() {
    let converter =
      Resampler::new_format_converter(2, 48000, AVSampleFormat::Fltp, AVSampleFormat::S16);
    assert!(
      converter.is_ok(),
      "Format converter creation failed: {:?}",
      converter.err()
    );
